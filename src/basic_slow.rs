...
#[command(
    long_about = "My first attempt at the Kuwahara filter.Same as the basic 
filter, just implemented worse and slightly less idiomatically. It is 4 times 
slower because of lack of caching."
)]
pub struct Args {
    #[arg(short, long)]
    input: PathBuf,
...
fn compute_quadrant(
    ...
    let mean = compute_window_mean_brightness(img, x, y, kernel_size);
    let var = compute_window_var(img, x, y, kernel_size, mean);
    let mean_color = compute_window_mean_color(img, x, y, kernel_size);
    return QuadrantResult { mean_color, var };
}

fn get_clamped_window_coords(
    x_start: i32,
    y_start: i32,
    kernel_size: u32,
    width: u32,
...
fn compute_window_mean_color(
    ...
    let ((x_start, x_end), (y_start, y_end)) =
        get_clamped_window_coords(left, top, kernel_size, width, height);
